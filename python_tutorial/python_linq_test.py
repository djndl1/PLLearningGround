...
class PinqTransformation(unittest.TestCase):
    ...
    def test_list_comprehensions_as_choose(self) -> None:
        print([f for f in self.fruit if len(f) > 5])
    ...
    def test_customized_helper_as_distinct(self) -> None:
        '''
            See https://github.com/morelinq/MoreLINQ/blob/master/MoreLinq/DistinctBy.cs
            for an implementation
        '''
        def distinct(it: Iterable) -> Generator:
            seen = set()
            for s in it:
                if s not in seen:
                    seen.add(s)
    ...
    def test_inner_join_the_hard_way(self) -> None:
        def inner_join(inner: Iterable[object], inner_key: Callable,
                       ...
                       result_selector: Callable) -> Generator:
...
class PinqReduction(unittest.TestCase):
    ...
    def test_min_max_by_key(self) -> None:
        min_fruit = min(self.fruit, key=len)
        max_fruit = max(self.fruit, key=len)
    ...
    def test_reversed_reduce_aggregate_right(self) -> None:
        def reduce_right(function: Callable, iterable, initializer=None):
        ...
        reduced = reduce_right(lambda x, y: f'({x}/{y})', range(1, 10), 10)
...
class PinqSetOperations(unittest.TestCase):
    ...
    def test_product_as_Cartesian(self) -> None:
        fruit2 = ['raspberry', 'peach', 'plum', '2']
        prod = product(self.fruit, fruit2)
